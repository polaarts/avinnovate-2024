...
const GenresAndArtists = ({ access_token }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const allGenres = response.data.items.flatMap(artist => artist.genres);
    ...
    if (!loading && artistNames.length > 0 && uniqueGenres.length > 0) {
      const postData = async () => {
        try {
          ...
          const responsePost = await axios.post('http://localhost:8000/recommend', jsonData);

          // Mostrar la respuesta del servidor
          if (responsePost.status === 200) {
            setResponseMessage(`Datos enviados exitosamente: ${JSON.stringify(responsePost.data)}`);
          } else {
            setResponseMessage(`Error al enviar los datos: Código ${responsePost.status}`);
          }
        } catch (err) {
  ...
  return (
    <h1>DATOS ENVIADOS CORRECTAMENTE AL SERVIDOR!</h1>
  );
};

...
import { ChevronLeft, ChevronRight, ShoppingCart, Info, Calendar, Clock, MapPin } from 'lucide-react'
...
const Navigation = () => (
  <nav className="bg-primary text-primary-foreground p-4">
    <div className="container mx-auto flex justify-between items-center">
      <h1 className="text-2xl font-bold">Teatro Espectacular</h1>
...
const SeatSelector2D = ({ onSeatSelect, selectedSeats }) => {
  const rows = 5
  const seatsPerRow = 8
  ...
  return (
    <Card className="w-full max-w-md">
      <CardHeader>
        ...
        <CardDescription>Haga clic en los asientos para seleccionarlos</CardDescription>
      </CardHeader>
      <CardContent>
                  ...
                  {Array.from({ length: seatsPerRow }).map((_, seatIndex) => {
                    ...
                    return (
                      <Button
                        ...
                        aria-label={`${isSelected ? 'Deseleccionar' : 'Seleccionar'} asiento ${seatNumber}`}
      ...
      <CardFooter>
        <p className="text-sm text-muted-foreground">Escenario en la parte superior</p>
      </CardFooter>
    </Card>
  )
}
...
const Seat = ({ position, color = "gray", isSelected }) => {
  const seatGeometry = useMemo(() => {
    ...
    const backrestVertices = [
      -backrestWidth/2, baseHeight, baseDepth/2,
      backrestWidth/2, baseHeight, baseDepth/2,
      backrestWidth/2, baseHeight + backrestHeight, baseDepth/2,
      -backrestWidth/2, baseHeight + backrestHeight, baseDepth/2,
      -backrestWidth/2, baseHeight, baseDepth/2 - backrestDepth,
      backrestWidth/2, baseHeight, baseDepth/2 - backrestDepth,
      backrestWidth/2, baseHeight + backrestHeight, baseDepth/2 - backrestDepth,
      -backrestWidth/2, baseHeight + backrestHeight, baseDepth/2 - backrestDepth,
    ]
    ...
    const leftArmrestVertices = [
      -baseWidth/2, baseHeight, -baseDepth/2,
      -baseWidth/2 + armrestWidth, baseHeight, -baseDepth/2,
      -baseWidth/2 + armrestWidth, baseHeight, baseDepth/2,
      -baseWidth/2, baseHeight, baseDepth/2,
      -baseWidth/2, baseHeight + armrestHeight, -baseDepth/2,
      -baseWidth/2 + armrestWidth, baseHeight + armrestHeight, -baseDepth/2,
      -baseWidth/2 + armrestWidth, baseHeight + armrestHeight, baseDepth/2,
      -baseWidth/2, baseHeight + armrestHeight, baseDepth/2,
    ]
...
const Band = () => (
  <group position={[0, 1, -6]}>
    <BandMember position={[-3, 0, 0]} color="red" />
    <BandMember position={[-1, 0, 0]} color="blue" />
    <BandMember position={[1, 0, 0]} color="green" />
    <BandMember position={[3, 0, 0]} color="yellow" />
  </group>
)
...
const TheaterView3D = ({ seatNumber }) => {
  ...
  return (
    <>
      ...
      <OrbitControls target={new THREE.Vector3(0, 2, -5)} enableZoom={false} enablePan={false} />
    </>
  )
}
...
export function TheaterSeating() {
  ...
  const handlePrevSeat = () => {
    setCurrentSeatIndex(prev => (prev - 1 + selectedSeats.length) % selectedSeats.length)
  }
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background text-foreground">
      ...
      <main className="flex-grow container mx-auto p-4">
        <h1 className="text-3xl font-bold mb-6">Reserva de Entradas: Concierto de Rock</h1>
        <div className="grid md:grid-cols-2 gap-8">
          <div>
            <Tabs defaultValue="2d" className="w-full">
              <TabsList className="grid w-full grid-cols-2">
                <TabsTrigger value="2d">Vista 2D</TabsTrigger>
                <TabsTrigger value="3d" disabled={selectedSeats.length === 0}>Vista 3D</TabsTrigger>
              </TabsList>
              <TabsContent value="2d">
                <SeatSelector2D onSeatSelect={handleSeatSelect} selectedSeats={selectedSeats} />
              </TabsContent>
              <TabsContent value="3d">
                <div className="w-full h-[400px] relative">
                  ...
                  <div className="absolute bottom-4 left-4 bg-white p-2 rounded shadow">
                    <p className="mb-2">Asiento {selectedSeats[currentSeatIndex]}</p>
                    <div className="flex items-center justify-between">
                      <Button size="sm" onClick={handlePrevSeat} disabled={selectedSeats.length <= 1}>
                        <ChevronLeft className="h-4 w-4" />
                      </Button>
                      <span className="mx-2">
                        {currentSeatIndex + 1} de {selectedSeats.length}
                      </span>
                      <Button size="sm" onClick={handleNextSeat} disabled={selectedSeats.length <= 1}>
          ...
          <div>
            <Card>
              <CardHeader>
                ...
                <CardDescription>Detalles del evento y asientos seleccionados</CardDescription>
              </CardHeader>
              <CardContent>
